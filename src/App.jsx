...
async function getUser(email) {
  const rows = await sbQ(`users?email=eq.${encodeURIComponent(email)}&select=*`);
  return rows?.[0] || null;
}
async function createUser(id, email, passwordHash, name) {
  await sbQ("users", { method: "POST", body: JSON.stringify({ id, email, password_hash: passwordHash, name }) });
}
...
// קמפיין + מאצ׳רים — משותפים
async function sbGet(table, id) {
  try {
    const rows = await sbQ(`${table}?id=eq.${id}&select=data`);
    return rows?.[0]?.data ?? null;
  } catch { return null; }
}
async function sbSet(table, id, data) {
  await sbQ(table, { method: "POST", body: JSON.stringify({ id, data, updated_at: new Date().toISOString() }), headers: { ...H, Prefer: "resolution=merge-duplicates" } });
}

// hash פשוט לסיסמה (לא קריפטוגרפי — מספיק לשימוש פנימי)
...
export default function App() {
  ...
  const [allDonors, setAllDonors] = useState([]); // כל התורמים של כולם — לדשבורד
  ...
  const [filters, setFilters] = useState({ circle: "ALL", status: "ALL", search: "", hideAssigned: false, hideDonated: false });
  ...
  // טען נתונים כשמשתמש נכנס
  useEffect(() => {
    if (!user) return;
    (async () => {
      setSyncing(true);
      const [mine, c, m] = await Promise.all([
        getDonors(user.id),
        sbGet("campaign", "main"),
        sbGet("matchers", "all"),
      ]);
      ...
      // טען את כל התורמים לדשבורד (של כל המשתמשים)
      const allRows = await sbQ("donors?select=data");
      setAllDonors(allRows?.map(r => r.data) || []);
  ...
  // עדכן allDonors כשמשתנה myDonors
  useEffect(() => {
    if (!user) return;
    setAllDonors(prev => {
      const others = prev.filter(d => !myDonors.find(m => m.id === d.id));
      return [...others, ...myDonors];
    });
  }, [myDonors]);
  ...
  const updateDonor = useCallback((id, upd) =>
    setMyDonors(p => p.map(d => d.id === id ? { ...d, ...upd } : d)), []);
  ...
  // מספרים לדשבורד — של כולם ביחד
  const raised = allDonors.filter(d => d.status === "DONATED").reduce((s, d) => s + (d.actualAmount || 0), 0);
  const pledged = allDonors.filter(d => d.status === "PLEDGED").reduce((s, d) => s + (d.estimatedAmount || 0), 0);
  const matching = matchers.reduce((s, m) => s + (m.amount || 0), 0);

  if (!user) return <LoginPage onLogin={(u) => { setUser(u); localStorage.setItem(SESSION_KEY, JSON.stringify(u)); }} showToast={showToast} toast={toast} />;
  ...
  if (selDonor) {
    const d = myDonors.find(x => x.id === selDonor);
  ...
  return (
    <div style={S.app}>
      ...
      <Header campaign={campaign} peak={peakMode} onPeak={() => setPeakMode(p => !p)} user={user} onLogout={logout} />
      <Nav page={page} setPage={setPage} />
      <main>
        {page === "dashboard" && <Dashboard donors={allDonors} campaign={campaign} raised={raised} pledged={pledged} matching={matching} peak={peakMode} onSelect={(id) => { if (myDonors.find(d => d.id === id)) setSelDonor(id); }} />}
        {page === "donors" && <DonorList donors={myDonors} setDonors={setMyDonors} onSelect={setSelDonor} showToast={showToast} filters={filters} setFilter={setFilter} />}
        {page === "import" && <ImportPage donors={myDonors} setDonors={setMyDonors} onSelect={setSelDonor} showToast={showToast} />}
        ...
        {page === "settings" && <SettingsPage campaign={campaign} setCampaign={setCampaign} donors={myDonors} showToast={showToast} />}
      </main>
    </div>
  );
}
...
// ─── LOGIN PAGE ───────────────────────────────────────────────
function LoginPage({ onLogin, showToast, toast }) {
  ...
  const doLogin = async () => {
    if (!email || !password) return showToast("מלא אימייל וסיסמה");
    setLoading(true);
    const u = await getUser(email.toLowerCase().trim());
    if (!u) { showToast("משתמש לא נמצא"); setLoading(false); return; }
    const h = await hashPass(password);
    if (h !== u.password_hash) { showToast("סיסמה שגויה"); setLoading(false); return; }
    onLogin({ id: u.id, name: u.name, email: u.email });
    setLoading(false);
  };

  const doRegister = async () => {
    if (!email || !password || !name) return showToast("מלא את כל השדות");
    if (password.length < 6) return showToast("סיסמה חייבת להיות לפחות 6 תווים");
    setLoading(true);
    const existing = await getUser(email.toLowerCase().trim());
    if (existing) { showToast("אימייל כבר קיים"); setLoading(false); return; }
    const h = await hashPass(password);
    const id = gid();
    await createUser(id, email.toLowerCase().trim(), h, name.trim());
    onLogin({ id, name: name.trim(), email: email.toLowerCase().trim() });
    setLoading(false);
  };
  ...
  return (
    <div style={{ ...S.app, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", padding: 20 }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 700, color: "#1A1A2E", marginBottom: 4 }}>Campaign Manager</div>
      <div style={{ fontSize: 14, color: "#888", marginBottom: 32 }}>Chabad House</div>
      ...
      <div style={{ ...S.card, width: "100%", maxWidth: 360 }}>
        <div style={{ display: "flex", marginBottom: 20, borderBottom: "2px solid #f0f0f0" }}>
          {[["login", "כניסה"], ["register", "הרשמה"]].map(([m, l]) => (
            <button key={m} style={{ flex: 1, padding: "10px 0", background: "none", border: "none", fontWeight: 700, fontSize: 14, cursor: "pointer", color: mode === m ? "#C8522A" : "#aaa", borderBottom: mode === m ? "2px solid #C8522A" : "2px solid transparent", marginBottom: -2, fontFamily: "Georgia,serif" }}
        ...
        {mode === "register" && (
          <div style={{ marginBottom: 12 }}>
            <label style={S.lbl}>שם</label>
            <input style={S.inp} placeholder="השם שלך" value={name} onChange={e => setName(e.target.value)} />
          </div>
        )}
        ...
        <button style={{ ...S.btn, opacity: loading ? 0.7 : 1 }} onClick={mode === "login" ? doLogin : doRegister} disabled={loading}>
...
// ─── HEADER ──────────────────────────────────────────────────
function Header({ campaign, peak, onPeak, user, onLogout }) {
  ...
  return (
    <header style={{ ...S.hdr, background: peak ? "linear-gradient(135deg,#8B1A1A,#C8522A)" : "linear-gradient(135deg,#1A1A2E,#2D2D5E)" }}>
      <div>
        ...
        <div style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", marginTop: 1 }}>👤 {user?.name}</div>
        {days !== null && days > 0 && <div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)" }}>{days} days to Peak</div>}
        {days === 0 && <div style={{ fontSize: 11, color: "#FFD700", fontWeight: 700 }}>🔥 PEAK DAYS!</div>}
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <button style={{ ...S.peakBtn, background: peak ? "#FFD700" : "rgba(255,255,255,0.18)", color: peak ? "#8B1A1A" : "#fff" }} onClick={onPeak}>{peak ? "⚡" : "Peak"}</button>
        <button style={{ ...S.peakBtn, background: "rgba(255,255,255,0.1)", color: "#fff", fontSize: 11 }} onClick={onLogout}>יציאה</button>
      </div>
    </header>
  );
}
      ...
      {totalEstimate > 0 && (
        <div style={{ ...S.card, background: "linear-gradient(135deg,#F0F7FF,#fff)", borderLeft: "4px solid #5B7EC8" }}>
          ...
          <div style={{ marginTop: 12, borderTop: "1px solid #eee", paddingTop: 10 }}>
            {[{ l: "First Circle", e: est1, c: "#C8522A" }, { l: "Second Circle", e: est2, c: "#B07D2E" }, { l: "Third Circle", e: est3, c: "#4A7B6F" }].filter(x => x.e > 0).map(x => (
          ...
          {forecastPct < 100 && <div style={{ marginTop: 10, fontSize: 13, color: "#C8522A", fontWeight: 600 }}>⚠️ חסר £{Math.max(0, campaign.goal - forecastTotal).toLocaleString()} להשלמת היעד</div>}
          {forecastPct >= 100 && <div style={{ marginTop: 10, fontSize: 13, color: "#2E7D32", fontWeight: 600 }}>✓ ההערכות מכסות את היעד!</div>}
        </div>
      )}
...
function DonorList({ donors, setDonors, onSelect, showToast, filters, setFilter }) {
  ...
  const activeFiltersCount = [filters.circle !== "ALL", filters.status !== "ALL", filters.search !== "", filters.hideAssigned, filters.hideDonated].filter(Boolean).length;
  ...
  return (
    <div style={S.pg}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
        <button style={{ ...S.toggleBtn, background: filters.hideAssigned ? "#C8522A" : "#f0f0f0", color: filters.hideAssigned ? "#fff" : "#555" }} onClick={() => { setFilter("hideAssigned", !filters.hideAssigned); clr(); }}>
          {filters.hideAssigned ? "✓" : ""} הסתר משויכים
        </button>
        <button style={{ ...S.toggleBtn, background: filters.hideDonated ? "#2E7D32" : "#f0f0f0", color: filters.hideDonated ? "#fff" : "#555" }} onClick={() => { setFilter("hideDonated", !filters.hideDonated); clr(); }}>
          {filters.hideDonated ? "✓" : ""} הסתר תורמים
        </button>
        {activeFiltersCount > 0 && (
          <button style={{ ...S.toggleBtn, background: "#1A1A2E", color: "#fff" }} onClick={() => { setFilter("circle", "ALL"); setFilter("status", "ALL"); setFilter("search", ""); setFilter("hideAssigned", false); setFilter("hideDonated", false); clr(); }}>נקה הכל ✕</button>
        )}
      </div>
        ...
        <div style={{ background: "#1A1A2E", borderRadius: 12, padding: "12px 14px", marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {["FIRST", "SECOND", "THIRD", "NOT_RELEVANT"].map(k => (
              <button key={k} style={{ background: CIRCLES[k].color, color: "#fff", border: "none", borderRadius: 20, padding: "7px 16px", fontSize: 13, fontWeight: 700, cursor: "pointer" }} onClick={() => assign(k)}>{CIRCLES[k].short}</button>
            ))}
          </div>
      ...
      {list.length === 0 && <div style={{ textAlign: "center", color: "#aaa", padding: "40px 20px", fontSize: 14 }}>אין תוצאות לסינון הנוכחי</div>}
    </div>
  );
}
...
function ImportPage({ donors, setDonors, onSelect, showToast }) {
  ...
  const doImport = () => {
    ...
    const nd = parsed.rows.map(row => {
      ...
      return { id, name: String(row[map.name] || "").trim() || "Unknown", phone: map.phone !== undefined ? String(row[map.phone] || "").trim() : "", email: map.email !== undefined ? String(row[map.email] || "").trim() : "", notes: map.notes !== undefined ? String(row[map.notes] || "").trim() : "", estimatedAmount: map.estimatedAmount !== undefined ? (Number(String(row[map.estimatedAmount]).replace(/[£$,]/g, "")) || 0) : 0, family: map.family !== undefined ? String(row[map.family] || "").trim() : "", circle: "UNASSIGNED", status: "NOT_CONTACTED", actualAmount: 0, lastContact: null, preCampaign: false, callLog: [], createdAt: tod() };
  ...
  if (step === "done") return (
    <div style={S.pg}>
      <div style={S.card}>
        ...
        <p style={{ fontSize: 14, color: "#666", marginBottom: 14 }}>{newIds.length} donors added.</p>
      ...
      <div style={S.card}>
        ...
        {newIds.slice(0, 20).map(id => { const d = donors.find(x => x.id === id); return d ? (<button key={id} style={S.ai} onClick={() => onSelect(id)}><div><div style={{ fontWeight: 700 }}>{d.name}</div><CB c={d.circle} /></div><div style={{ fontSize: 12, color: "#aaa" }}>{d.phone}</div></button>) : null; })}
      </div>
    </div>
  );
  ...
  return (
    <div style={S.pg}>
      <div style={S.card}>
        ...
        <p style={{ fontSize: 14, color: "#666", marginBottom: 20, lineHeight: 1.6 }}>Upload a <strong>CSV file</strong> with columns: Name, Phone, Email, Notes, Estimated Amount</p>
      ...
      <div style={S.card}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
          {Object.entries(CIRCLES).map(([k, v]) => { const n = donors.filter(d => d.circle === k).length; return n ? <span key={k} style={{ padding: "4px 12px", borderRadius: 99, background: v.bg, color: v.color, fontSize: 13, fontWeight: 700 }}>{v.short}: {n}</span> : null; })}
        </div>
      </div>
    </div>
  );
...
function MatchPage({ matchers, setMatchers, campaign, matching, showToast }) {
  ...
  const add = () => { if (!name.trim() || !Number(amt)) return; setMatchers(p => [...p, { id: gid(), name: name.trim(), amount: Number(amt), date: tod() }]); showToast(`${name} added`); setName(""); setAmt(""); };
  return (
    <div style={S.pg}>
      <div style={{ ...S.card, borderLeft: "4px solid #5B7EC8" }}>
        ...
        <div style={{ marginTop: 8, fontSize: 13, fontWeight: 700, color: rem > 0 ? "#C8522A" : "#2E7D32" }}>{rem > 0 ? `£${rem.toLocaleString()} more matching needed` : "✓ Matching target reached!"}</div>
      </div>
      <div style={S.card}>
...
function SettingsPage({ campaign, setCampaign, donors, showToast }) {
  ...
  const exportList = () => {
    ...
    const lines = ["CALL LIST FOR TODAY", "=".repeat(44), "", ...donors.filter(d => !["DONATED", "NOT_RELEVANT"].includes(d.status)).sort((a, b) => (ord[a.circle] ?? 3) - (ord[b.circle] ?? 3)).map(d => `${CIRCLES[d.circle]?.short || "?"} | ${d.name.padEnd(25)} | ${(d.phone || "—").padEnd(15)} | Est: £${String(d.estimatedAmount || 0).padStart(6)} | ${STATUSES.find(s => s.key === d.status)?.label || "?"}`)];
...
function DonorCard({ donor, onUpdate, onBack, showToast }) {
  ...
  const saveNote = () => { if (!note.trim()) return; onUpdate(donor.id, { callLog: [...(donor.callLog || []), { date: tod(), note: note.trim() }], lastContact: tod() }); showToast("Note saved"); setNote(""); };
  const addDon = () => { const a = Number(dam); if (!a) return; onUpdate(donor.id, { actualAmount: (donor.actualAmount || 0) + a, status: "DONATED", lastContact: tod() }); showToast(`£${a.toLocaleString()} recorded ✓`); setDam(""); };
  const openWA = () => { const t = (WA[donor.circle] || WA.THIRD).replace("[NAME]", donor.name.split(" ")[0]); const p = (donor.phone || "").replace(/\D/g, ""); window.open(`https://wa.me/${p || ""}?text=${encodeURIComponent(t)}`, "_blank"); };
        ...
        <div style={{ ...S.card, borderLeft: `4px solid ${ci.color}` }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}><CB c={donor.circle} /><SB s={donor.status} />{donor.preCampaign && <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 99, background: "#EBF5F2", color: "#4A7B6F", fontWeight: 600 }}>Pre-Campaign</span>}</div>
            </div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {["FIRST", "SECOND", "THIRD", "NOT_RELEVANT"].map(k => (
              <button key={k} style={{ padding: "9px 16px", borderRadius: 20, border: `2px solid ${CIRCLES[k].color}`, background: donor.circle === k ? CIRCLES[k].color : CIRCLES[k].bg, color: donor.circle === k ? "#fff" : CIRCLES[k].color, fontWeight: 700, fontSize: 13, cursor: "pointer" }}
                onClick={() => { onUpdate(donor.id, { circle: k }); showToast(CIRCLES[k].label); }}>{CIRCLES[k].label}</button>
            ))}
          </div>
        </div>
        <div style={S.card}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {STATUSES.map(s => (
              <button key={s.key} style={{ padding: "9px 14px", borderRadius: 20, border: "none", background: donor.status === s.key ? s.color : "#f0f0f0", color: donor.status === s.key ? "#fff" : s.color, fontWeight: 700, fontSize: 13, cursor: "pointer" }}
                onClick={() => { onUpdate(donor.id, { status: s.key, lastContact: tod() }); showToast(s.label); }}>{s.icon} {s.label}</button>
            ))}
          </div>
        </div>
        <div style={S.card}>
...
function PB({ pct, c, h = 12 }) {
  return <div style={{ background: "#eee", borderRadius: 99, overflow: "hidden", height: h }}><div style={{ width: `${Math.max(0, pct)}%`, height: "100%", background: c, borderRadius: 99, transition: "width 0.5s" }} /></div>;
}
function SC({ l, v, a }) {
  return <div style={{ background: "#fff", borderRadius: 12, padding: "12px 14px", borderTop: `3px solid ${a}`, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}><div style={{ fontSize: 10, color: "#bbb", textTransform: "uppercase", letterSpacing: 1 }}>{l}</div><div style={{ fontSize: 20, fontWeight: 700, color: a, marginTop: 2 }}>{v}</div></div>;
}
function CB({ c }) {

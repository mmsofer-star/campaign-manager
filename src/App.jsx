...
// ─── STORAGE (localStorage — works in browser + local dev) ───
function sLoad(k, d) {
  try {
...
export default function App() {
  const [page, setPage] = useState("dashboard");
  const [donors, setDonors] = useState(() => sLoad("cf_donors", []));
  const [campaign, setCampaign] = useState(() => sLoad("cf_campaign", DEFAULT_CAMPAIGN));
  const [matchers, setMatchers] = useState(() => sLoad("cf_matchers", []));
  const [selDonor, setSelDonor] = useState(null);
  const [peakMode, setPeakMode] = useState(false);
  const [toast, setToast] = useState(null);
  const toastRef = useRef();
  ...
  useEffect(() => { sSave("cf_donors", donors); }, [donors]);
  useEffect(() => { sSave("cf_campaign", campaign); }, [campaign]);
  useEffect(() => { sSave("cf_matchers", matchers); }, [matchers]);
...
function Dashboard({ donors, campaign, raised, pledged, matching, peak, onSelect }) {
  ...
  if (donors.length === 0) return (
    <div style={S.pg}>
      <div style={{ ...S.card, textAlign: "center", padding: "40px 20px" }}>
        <div style={{ fontSize: 48, marginBottom: 12 }}>🕍</div>
        <div style={{ fontSize: 20, fontWeight: 700, color: "#1A1A2E", marginBottom: 8 }}>Welcome to Campaign Manager</div>
        <div style={{ fontSize: 14, color: "#888", lineHeight: 1.7, marginBottom: 20 }}>
          Start by importing your contact list via the <strong>Import</strong> tab,<br />
          or configure your campaign in <strong>Settings</strong>.
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ background: "#FFF0EB", borderRadius: 10, padding: 12, fontSize: 13, color: "#C8522A", textAlign: "left" }}>📤 <strong>Import</strong> — upload a CSV of contacts</div>
          <div style={{ background: "#FFF8EB", borderRadius: 10, padding: 12, fontSize: 13, color: "#B07D2E", textAlign: "left" }}>⚙️ <strong>Settings</strong> — set your goal, peak dates, targets</div>
          <div style={{ background: "#EBF5F2", borderRadius: 10, padding: 12, fontSize: 13, color: "#4A7B6F", textAlign: "left" }}>🤝 <strong>Match</strong> — add your matching donors</div>
        </div>
      </div>
    </div>
  );
  ...
  if (step === "done") return (
    <div style={S.pg}>
      <div style={S.card}>
        <div style={{ fontSize: 40, marginBottom: 8 }}>✅</div>
        <div style={S.ct}>Import Complete</div>
        <p style={{ fontSize: 14, color: "#666", marginBottom: 14 }}>{newIds.length} donors added. Now assign them to circles in the Donors tab.</p>
        <div style={{ background: "#f5f5f5", borderRadius: 10, padding: 12, marginBottom: 16, fontSize: 13, color: "#555", lineHeight: 1.7 }}>
          <strong>Tip:</strong> Select multiple donors → press <strong>1</strong> (First), <strong>2</strong> (Second), <strong>3</strong> (Third), or <strong>0</strong> (N/R)
        </div>
        <button style={S.btn} onClick={() => { setStep("up"); setParsed(null); setNewIds([]); }}>Import Another File</button>
      </div>
      <div style={S.card}>
...
function SettingsPage({ campaign, setCampaign, donors, showToast }) {
  ...
  return (
    <div style={S.pg}>
      ...
      <div style={S.card}>
        <div style={S.ct}>Data Management</div>
        <div style={{ fontSize: 13, color: "#aaa", marginBottom: 12 }}>All data stored locally in your browser (localStorage).</div>
        <button style={{ width: "100%", padding: 12, background: "#fff5f5", border: "1px solid #ffcccc", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", color: "#C8522A" }}
          onClick={() => {
            if (window.confirm("Reset ALL data? This cannot be undone.")) {
              localStorage.removeItem("cf_donors");
              localStorage.removeItem("cf_campaign");
              localStorage.removeItem("cf_matchers");
